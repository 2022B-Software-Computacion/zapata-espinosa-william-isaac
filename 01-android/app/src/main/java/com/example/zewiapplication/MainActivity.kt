...
class MainActivity : AppCompatActivity() {
        ...
        if(result.resultCode == RESULT_OK){
            if(result.data != null){
                if(result.data!!.data != null){
                    ...
                    cursor?.moveToFirst()
                    val indiceTeefono = cursor?.getColumnIndex(
    ...
    fun abrirActividadConParamentros(
        clase: Class<*>
    ){
        val intentExplicito = Intent(this, clase)
        //Enviar paramentros (solamente variables primitivas)
        intentExplicito.putExtra("nombre", "William")
        intentExplicito.putExtra("apellido", "Zapata")
        intentExplicito.putExtra("edad", 23)
        intentExplicito.putExtra(
            "entrenadorPrincipal",
            BEntrenador(1,"William", "Paleta")
        )
